...
void AIMind::update(HUD* hud, Intention intention, float seconds, Waypoint* waypoints[], Waypoint* checkpoints[]){
	...
	switch(racerType){
		case PLAYER:
			...
			if(hud->getSelectedAbility() == SPEED && intention.aPressed && !speedBoost->onCooldown()){
				speedBoost->startCooldownTimer();
			}

			if(hud->getSelectedAbility() == LASER && intention.aPressed){
				racer->fireLaser();
			}
			...
			racer->steer(seconds, intention.steering);
