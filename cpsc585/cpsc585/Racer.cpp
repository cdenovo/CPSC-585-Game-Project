...
void Racer::serialize(char buff[])
{
	...
	data.rotation = body->getRotation();
	data.linearVelocity = body->getLinearVelocity();
	data.angularVelocity = body->getAngularVelocity();
	data.wheelFL = wheelFL->body->getPosition();
	data.wheelFR = wheelFR->body->getPosition();
	data.wheelRL = wheelRL->body->getPosition();
...
void Racer::unserialize(RacerData *data)
{
	body->setPosition(data->position);
	body->setRotation(data->rotation);
	body->setLinearVelocity(data->linearVelocity);
	body->setAngularVelocity(data->angularVelocity);
	wheelFL->body->setPosition(data->wheelFL);
	wheelFR->body->setPosition(data->wheelFR);
	wheelRL->body->setPosition(data->wheelRL);
	wheelRR->body->setPosition(data->wheelRR);
	lookDir = data->lookDir;
	lookHeight = data->lookHeight;
	if(!data->laserCoolDown)
	{
		laserReady = true;
	}
	else if(laserReady)
