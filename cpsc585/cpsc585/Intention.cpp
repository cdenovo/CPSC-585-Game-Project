...
{
	//Get boolean values
	xPressed = bitstr[0] & 0x80;
	yPressed = bitstr[0] & 0x40;
	aPressed = bitstr[0] & 0x20;
	bPressed = bitstr[0] & 0x10;
	selectPressed = bitstr[0] & 0x08;
	startPressed = bitstr[0] & 0x04;
	lbumpPressed = bitstr[0] & 0x02;
	rbumpPressed = bitstr[0] & 0x01;
 ...
 */
string Intention::toStr()
{
	stringstream ss;
	ss << "xPressed: " << xPressed << "\nyPressed: " << yPressed << endl;
	ss << "aPressed: " << aPressed << "\nbPressed: " << bPressed << endl;
	ss << "selectPressed: " << selectPressed << "\nstartPressed: " << startPressed << endl;
	ss << "lbumpPressed: " << lbumpPressed << "\nrbumpPressed: " << rbumpPressed << endl;

	ss << "rightStickX: " << rightStickX << "\nrightStickY: " << rightStickY << endl;
	ss << "leftStick: " << leftStick << endl;
	ss << "rightTrig: " << rightTrig << "\nleftTrig: " << leftTrig << endl;
