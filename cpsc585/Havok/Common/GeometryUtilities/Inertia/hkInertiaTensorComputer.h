...
struct hkpMassProperties
{
	...
	hkpMassProperties() :	m_volume(0.0f),		m_mass(0.0f)	{		m_centerOfMass.setZero();		m_inertiaTensor.setZero();	}
...
/// Mass distribution, assumes a mass of 1.0f
struct hkMassDistribution: public hkpMassProperties
{
	hkMassDistribution(): hkpMassProperties()
	{
		m_mass = 1.0f;
	}
	~hkMassDistribution()
	{
		HK_ASSERT( 0xf0342da3, m_mass == 1.0f );
	}
...
class hkInertiaTensorComputer
{
	public:
		...
		static hkResult HK_CALL computeBoxVolumeMassProperties(const hkVector4& halfExtents, hkReal mass, hkpMassProperties& result);
			...
			/// Creates mass properties given box halfextents, considered as a volume of uniform density.
			/// Returns the volume of the box and the inertia diagonal
		static hkReal HK_CALL computeBoxVolumeMassPropertiesDiagonalized(const hkVector4& halfExtents, hkReal mass, hkVector4 &inertiaDiagonal);
