...
class hkQemSimplifier: public hkReferencedObject
{
	public:
		...
		void startGroup(int attribSize, int positionOffset, hkReal positionToAttribScale);
			/// Add a triangle
		void addTriangle(const int positionIndices[], const int attributeIndices[], hkReal weight = 1.0f);
		...
		void debugDisplay();
			...
			/// Must be set for open edged geometry to be rpocessed correctly
		...
		hkBool getEnableOpenEdgeGeometry() const { return m_enableOpenEdgeGeometry; }
			...
			/// Set the 'open edge' scale. The QEM will be scaled depending on this, and the amount of open edges (ie not connected to another triangle).
			/// The effect of making this greater is to bias such triangles that have open edges, are less likely to be removed 
		void setOpenEdgeScale(hkReal scale) { m_openEdgeScale = scale; }
			/// Get the open edge scale
		hkReal getOpenEdgeScale() const { return m_openEdgeScale; }
	...
	protected:
		...
		hkReal m_openEdgeScale;													///< Scaling used for open edge processing
