...
class hkMeshSimplifierConverter
{
	...
	HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_BASE,hkMeshSimplifierConverter);
		...
		hkResult initMesh(const hkMeshShape* shape, hkQemSimplifier& simplifier, hkFindUniquePositionsUtil& positionUtil);
			/// Add lots of meshes to simplify simultaneously
		hkResult initMeshes(const hkMeshShape*const* srcMeshes, int numMeshes, hkQemSimplifier& simplifier, hkFindUniquePositionsUtil& positionUtil);
		...
		hkMeshShape* simplifyCoplanar(hkMeshSystem* system, hkMeshShape* meshShape, hkReal maxCoplanarError = 1e-6f, hkBool allowOpen = true);
