...
class hkQemVertexContractionController: public hkQemSimplifier::ContractionController
{
	public:
		...
		virtual hkBool32 allowContraction(const EdgeContraction& contraction);
			...
			/// Enable a vertex 
		void setVertexCanContract(int index, hkBool enable);

			/// Returns true if the vertex can contract
		hkBool32 canVertexContract(int index) const;
		...
		void setVertexCanContractFromAabb(const hkArray<hkVector4>& positions, const hkAabb& aabb, hkBool enable);
