...
inline hkVector4										TRIANGULATOR_TYP::getEffectiveAccuracy() const
{
	hkVector4	d,td,acc;
	acc.setZero();
	d.set(1,0,0,0);	m_localToWorld.transformDirection(d,td); acc.setComponent<0>(td.length<3>());
	d.set(0,1,0,0);	m_localToWorld.transformDirection(d,td); acc.setComponent<1>(td.length<3>());
	return acc;
}
...
HKGP_FORCE_INLINE typename TRIANGULATOR_TYP::Edge			TRIANGULATOR_TYP::conformEdge(const Edge& edge)
{
	...
	hkResult expandRes = m_stack.reserve( m_stack.getSize() + isNaked ? 2 : 4 );
		...
		case	Insertion::CROSSING_CONSTRAINED_EDGE:
			{
				...
				if(!split)
				{
					...
					for(int i=1,ni=hkMath::min2((hkInt32)cfgSearchSteps,(hkInt32)(4+hkMath::max2(absDx,absDy)*8));i<ni;++i)
					{
						...
						{
							hkVector4	isec;
							hkSimdReal sf; sf.setFromFloat(fraction);
							isec.setInterpolate(isecPoint,endPoints[s],sf);
							hkReal		sqd=isec.distanceToSquared(isecPoint).getReal();
...
// 				}
				for(int i=0;i<3;++i)
				{
					...
					if(ne.isValid()&&(nt->m_region==nullregion))
					{
						hkBool constrained = ne.isConstrained() || ne.link().isConstrained();
