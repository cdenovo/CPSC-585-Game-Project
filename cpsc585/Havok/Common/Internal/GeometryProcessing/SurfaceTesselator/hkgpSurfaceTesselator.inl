...
{	
	...
	for(int i=0;i<numTriangles;++i)
	{
		...
		if(c == 0x08)
		{
			...
			edgeLengths.set(t->m_vertices[0]->m_position.distanceToSquared(t->m_vertices[1]->m_position),
							t->m_vertices[1]->m_position.distanceToSquared(t->m_vertices[2]->m_position),
							t->m_vertices[2]->m_position.distanceToSquared(t->m_vertices[0]->m_position));
			c = 1<<edgeLengths.getIndexOfMaxAbsComponent<3>();
		}
		if(c)
		{
			if(c == 0x10)
...
inline void									TESSELATOR_TYP::splitTriangleEdges(const Triangle* triangle,Edge** edges,int edgesToSplit)
{
	...
	for(int i=0;i<3;++i)
	{
		...
		{
			hkVector4	midPoint;	midPoint.setInterpolate(edges[i]->m_vertices[0]->m_position,edges[i]->m_vertices[1]->m_position,edges[i]->m_fraction);
