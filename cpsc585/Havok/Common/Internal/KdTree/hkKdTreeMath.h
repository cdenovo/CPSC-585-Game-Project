...
public:
	...
	static HK_FORCE_INLINE hkVector4Comparison rayBundleAabbCheck(const hkFourTransposedPoints& orig, const hkFourTransposedPoints& invDir, hkVector4Parameter _tNear, hkVector4Parameter _tFar, const hkVector4* aabbData);
	...
	/// Used for checking whether a ray bundle hits the specified AABB.
	/// aabbData stored in 6 vectors, each with all 4 components the same, as produced by getAabbVecsFromSplitArray
	static hkVector4Comparison _rayBundleAabbCheck(const hkFourTransposedPoints& orig, const hkFourTransposedPoints& invDir, hkVector4Parameter _tNear, hkVector4Parameter _tFar, const hkVector4* aabbData);
