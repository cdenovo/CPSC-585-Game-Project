...
public:
	...
	static hkSplitPrimitivesStatus HK_CALL splitPrimitives( const hkKdTreeBuildInput::WorkingSet& buffersIn, int startPrimIdx, int endPrimIdx, SplitPrimitivesResults& results, 
		HK_PAD_ON_SPU(int)& numAvailableEmptyNodes, hkKdTreeBuildInput::WorkingSet& buffersOut, const hkKdTreeProjectedEntry& bounds, hkVector4ComparisonParameter allowedEmptySplits);
	
	static int HK_CALL buildTreeRecursive( const hkKdTreeBuildInput::BuildRecursiveInput& input, hkKdTreeBuildInput::TreeStream* streamOut, hkKdTreeProjectedEntry* peOut, HK_PAD_ON_SPU(int)& numAllocatedNodes, HK_PAD_ON_SPU(int)& numEmptyNodesRemaining);
	...
	static hkSplitPrimitivesStatus HK_CALL splitPrimitivesExactly( const hkKdTreeBuildInput::WorkingSet& buffersIn, int startPrimIdx, int endPrimIdx, 
		SplitPrimitivesResults& results, hkKdTreeBuildInput::WorkingSet& buffersOut, const hkKdTreeProjectedEntry& bounds );

	static void HK_CALL splitTreeBuilding( const hkKdTreeBuildInput::WorkingSet& buffersIn, hkKdTreeNode* treeOut, int treeSize, int numPrimitives,  hkKdTreeBuildInput::BuildRecursiveInput* subJobs, int subTreeSizes[4], const hkKdTreeProjectedEntry& bounds );

	static void HK_CALL writeEmptyLeaf(hkKdTreeNode* treeRoot, int nodeIdx, class hkKdTreeBuilderSparseWriter* writer);
	static void HK_CALL writeSinglePrimitiveToLeaf(hkKdTreeNode* treeRoot, hkKdTreeProjectedEntry* entriesIn, hkKdTreeProjectedEntry* projEntOut, int nodeIdx, int primIdx, hkKdTreeBuilderSparseWriter* writer);
	...
	static int HK_CALL writeMultiplePrimitivesToLeaf(hkKdTreeNode* HK_RESTRICT treeRoot, const hkKdTreeProjectedEntry* entriesIn, hkKdTreeProjectedEntry* HK_RESTRICT projEntOut, int nodeIdx, int primIdx, int numPrims, hkKdTreeBuilderSparseWriter* writer);
};


// It's safe to add __restrict on the write iterators, since they're write-only.
typedef const hkKdTreeProjectedEntry* ProjectedEntryDirectReadIterator;
typedef hkKdTreeProjectedEntry* HK_RESTRICT ProjectedEntryDirectWriteIterator;
...
public:
	...
	void putProjEntToMainMemory(hkKdTreeProjectedEntry* base, int peIdx, const hkKdTreeProjectedEntry& peToWrite);
