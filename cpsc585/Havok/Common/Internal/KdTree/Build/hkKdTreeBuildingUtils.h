...
public:
	...
	static void HK_CALL buildTree( hkKdTreeBuildInput::WorkingSet& buffers, hkKdTreeNode* treeOut, hkKdTreeProjectedEntry* projEntOut, int treeOutSize, int numPrims, int _numAvailableEmptyNodes, int& maxDepthOut );
	...
	static void HK_CALL completeDistributedBuild( hkKdTree* treeInOut, int numPrimitives, int* depths, int* subJobSizes );
		...
		/// For fastbuilds
	static void HK_CALL projectPrimitives(hkKdTreeBuildInput::Mediator* HK_RESTRICT mediator, int startIdx, int endIdx, hkKdTreeProjectedEntry* HK_RESTRICT output );
