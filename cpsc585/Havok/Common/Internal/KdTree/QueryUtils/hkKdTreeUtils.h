...
public:
	...
	static hkBool HK_CALL isValidRayBundle(const hkFourTransposedPoints& fromBundle, const hkFourTransposedPoints& toBundle, hkVector4Comparison::Mask activeRayMask);
	...
	// Ray bundle versions
	static hkBool HK_CALL castRayBundleIterative( const hkKdTree* tree, const hkFourTransposedPoints& start, const hkFourTransposedPoints& end, hkKdTreeBundleCastCollector* collector, hkVector4Comparison::Mask rayMask = hkVector4Comparison::MASK_XYZW);
...
HK_FORCE_INLINE const hkKdTreeNode* HK_CALL hkGetNodeFromMainMemory(const hkKdTreeNode* ppuNode, hkBool waitForCompletion = true);
HK_FORCE_INLINE const hkKdTreeNode* HK_CALL hkGetLeftMainMemoryAddr(const hkKdTreeNode& spuNode, const hkKdTreeNode* ppuNode);
HK_FORCE_INLINE const hkKdTreeNode* HK_CALL hkGetRightMainMemoryAddr(const hkKdTreeNode& spuNode, const hkKdTreeNode* ppuNode);
HK_FORCE_INLINE const hkKdTreeProjectedEntry* HK_CALL hkGetProjectedEntry(const hkKdTreeNode& spuNode, const hkKdTreeProjectedEntry* base, int primIdx = 0);
HK_FORCE_INLINE void HK_CALL hkWaitForDma();
