...
namespace hkMemoryInitUtil
{
	...
	hkMemoryRouter* HK_CALL initFreeListLargeBlock(hkMemoryAllocator *memoryAllocator, const hkMemorySystem::FrameInfo& info, const hkFreeListAllocator::Cinfo* cinfo = HK_NULL);
	...
	hkMemoryRouter* HK_CALL initFreeList(hkMemoryAllocator *memoryAllocator, hkMemoryAllocator::ExtendedInterface* memoryAllocatorExtended, const hkMemorySystem::FrameInfo& info, const hkFreeListAllocator::Cinfo* cinfo = HK_NULL);
	...
	hkMemoryRouter* HK_CALL initOptimizer(hkMemoryAllocator *memoryAllocator, const hkMemorySystem::FrameInfo& info);
		...
		/// such as initFreeList - that uses a hkFreeListAllocator as the heap allocator). The heapInterface provides
		...
		/// the solvers memory block is allocated from the memoryAllocator.
