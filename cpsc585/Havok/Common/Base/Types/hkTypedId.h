...
struct hkTypedId
{
	...
	/// Empty constructor
	HK_ALWAYS_INLINE hkTypedId(){}

	/// Only allow for explicit or default construction.
	explicit HK_FORCE_INLINE			hkTypedId(int v) : m_value((TYPE)v) {}

	HK_FORCE_INLINE hkTypedId& operator =(hkTypedId other) { m_value = other.value(); return *this; }
	...
	HK_ALWAYS_INLINE TYPE				value() const						{ return m_value; }

	/// Read-write access to the actual index value.
	HK_ALWAYS_INLINE TYPE&				value()								{ return m_value; }
	
	/// Check if the ID is valid ( != INVALID_VALUE / != invalid() )
	HK_FORCE_INLINE bool				isValid() const						{ return m_value != INVALID_VALUE; }

	/// Set this ID to invalid.
	HK_FORCE_INLINE void				invalidate()						{ m_value = INVALID_VALUE; }
	
	/// Get an invalid ID ( equivalent to hkTypedId() ).
	static HK_FORCE_INLINE hkTypedId	invalid()							{ return hkTypedId(INVALID_VALUE); }
