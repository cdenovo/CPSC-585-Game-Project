...
class hkAabbUtil
{
	public:
		...
		static void HK_CALL				calcAabb( const hkReal* vertexArray, int numVertices, int striding, hkAabb& aabbOut );
		...
		static HK_FORCE_INLINE void HK_CALL		calcAabb( const hkTransform& BvToWorld, const hkVector4& halfExtents, const hkVector4& center, hkSimdRealParameter extraRadius, hkAabb& aabbOut);

			/// Calculates an AABB from an OBB specified by a transform, a center, and an halfExtents vector.
		static HK_FORCE_INLINE void HK_CALL		calcAabb( const hkTransform& BvToWorld, const hkVector4& halfExtents, const hkVector4& center, hkAabb& aabbOut);

			/// Calculates an AABB from an OBB specified by a transform, and an halfExtents vector.
		static HK_FORCE_INLINE void HK_CALL		calcAabb( const hkTransform& BvToWorld, const hkVector4& halfExtents, hkSimdRealParameter extraRadius, hkAabb& aabbOut);
		...
		static void HK_CALL				calcAabb(const hkTransform& BvToWorld, const hkAabb& aabb, hkAabb& aabbOut);

			/// Calculates an AABB from center and inner radius.
		static HK_FORCE_INLINE void HK_CALL		calcAabb( const hkVector4& center, hkSimdRealParameter innerRadius, hkAabb& aabbOut);

		static HK_FORCE_INLINE void HK_CALL		transformAabbIntoLocalSpace( const hkTransform& localToWorld, const hkAabb& aabb, hkAabb& aabbOut );

			/// Get the outer radius of an AABB.
		static HK_FORCE_INLINE hkReal HK_CALL	getOuterRadius(const hkAabb& aabb);

			/// Get the distance squared from point to an AABB, with point inside AABB distance == 0.
		static HK_FORCE_INLINE hkReal			distanceSquared(const hkAabb& a,const hkVector4& b);

			/// Get a vertex of an AABB. (with index bits set for positive as follow: Z | Y | X)
		static HK_FORCE_INLINE void				getVertex(const hkAabb& aabb, int index, hkVector4& vertexOut);
