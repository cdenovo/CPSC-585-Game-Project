...
struct hkAabbHalf
{
	...
	HK_FORCE_INLINE	void	pack(const hkAabb& aabb)
	{
		...
		hkIntVector			iminD; iminD.setAnd(	hkIntVector::cast(aabb.m_min), *rdown.v);
		hkIntVector			iminU; iminU.setAddU32(	hkIntVector::cast(aabb.m_min), *rup.v);
		hkIntVector			imaxD; imaxD.setAnd(	hkIntVector::cast(aabb.m_max), *rdown.v);
		hkIntVector			imaxU; imaxU.setAddU32(	hkIntVector::cast(aabb.m_max), *rup.v);
	...
	HK_FORCE_INLINE void	unpackUnaligned(hkAabb& aabb) const
	{
		...
		aabb.m_max = hkIntVector::cast(imax);
		aabb.m_min = hkIntVector::cast(imin);
	}
