...
struct hkSlot
{
	...
	HK_FORCE_INLINE hkSlot*		getNext() const				{ return reinterpret_cast<hkSlot*>(reinterpret_cast<hkUlong>(m_next) & ~hkUlong(1)); }
...
#define HK_DECLARE_SIGNAL(_name_ , ...) \
	...
	{ \
		...
		{ \
			return memberSubscribe( instance , &CLASS_TYPE::on##_name_ , slotname ); \
		} \
	}

/// Subscrive to a signal using the implicit method match of a class instance ('on_' + signal type name , i.e. 'hkMySignal' -> 'on_hkMySignal').
