...
struct HK_SIGNAL_TYPE : hkSignal
{
	...
	struct Slot : hkSlot
	{
		...
		HK_FORCE_INLINE Slot(hkSlot*& slots, TYPE* object, const char* name) : hkSlot(slots, object, name) {}
	...
	struct GlobalSlot : Slot
	{
		...
		HK_FORCE_INLINE		GlobalSlot(hkSlot*& slots, METHOD method, const char* name) : Slot(slots, (void*)HK_NULL, name), m_method(method) {}
	...
	struct MemberSlot : Slot
	{
		...
		HK_FORCE_INLINE		MemberSlot(hkSlot*& slots, TYPE* object, METHOD method, const char* name) : Slot(slots, object, name), m_method(method) {}
	...
	template <typename METHOD>
	HK_NEVER_INLINE Slot*	globalSubscribe(METHOD method, const char* name)
	{
	...
	template <typename TYPE, typename METHOD>
	HK_NEVER_INLINE Slot*	memberSubscribe(TYPE* object, METHOD method, const char* name )
	{
		#ifdef HK_SIGNAL_DEBUG
	...
	// Fire the signal.
	HK_NEVER_INLINE	void	fire(HK_SIGNAL_ARGUMENTS) { _fire(HK_SIGNAL_NAMES); }
