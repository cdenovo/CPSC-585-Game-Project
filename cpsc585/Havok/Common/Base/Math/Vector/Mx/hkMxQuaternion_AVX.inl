...
{
	hkMxQuaternion_Implementation::gatherQH<M, 0, byteAddressIncrement>::apply(this->m_vec.v, &(base->m_vec.m_quad));
}
...
{
	hkMxQuaternion_Implementation::scatterQH<M, 0, byteAddressIncrement>::apply(this->m_vec.v, &(base->m_vec.m_quad));
}
...
{
	hkMxQuaternion_Implementation::setMulQH<((M+1)>>1)>( this->m_vec.v, p.m_vec.v, q.m_vec.v);
}
...
{
	hkMxQuaternion_Implementation::setMulInverseQH<((M+1)>>1)>( this->m_vec.v, p.m_vec.v, q.m_vec.v);
}
...
HK_FORCE_INLINE void hkMxQuaternion<M>::setInverse(hkMxQuaternionParameter p)
{
	...
	hkMxQuaternion_Implementation::setInverseQH<((M+1)>>1)>( this->m_vec.v, p.m_vec.v, *(const __m256*)&mask);
}
...
HK_FORCE_INLINE static void apply(const __m256* v, hkRotation* base)
{
	hkQuaternion q0,q1;
	q0.m_vec.m_quad = _mm256_extractf128_ps(v[N], 0x0);
	q1.m_vec.m_quad = _mm256_extractf128_ps(v[N], 0x1);

	hkRotation* r0 = hkAddByteOffset(base, (2*N) * byteAddressIncrement);
	hkRotation* r1 = hkAddByteOffset(base, (2*N+1) * byteAddressIncrement);
...
HK_FORCE_INLINE static void apply(const __m256* v, hkRotation* base)
{
	...
	hkVector4Util::convert4QuaternionsToRotations( (const hkQuaternion*)q,  // out of line call
...
HK_FORCE_INLINE static void apply(const __m256* v, hkRotation* base)
{
	...
	hkRotation* r = hkAddByteOffset(base, (2*N) * byteAddressIncrement);
	r->set(q);	// out of line
} };
}
template <int M>
...
{
	hkMxQuaternion_Implementation::scatterQHR<M, 0, byteAddressIncrement>::apply(this->m_vec.v, base);
}
