...
public:
	...
	HK_FORCE_INLINE void pack(hkMxVectorParameter v0, hkMxVectorParameter v1 );
	/// Store \a v by reducing precision and interleaving with existing second vector (untouched).
	HK_FORCE_INLINE void packFirst(hkMxVectorParameter v);
	/// Store \a v by reducing precision and interleaving with existing first vector (untouched).
	HK_FORCE_INLINE void packSecond(hkMxVectorParameter v);

	/// Unpack and sort back into \a v0 and \a v1.
	HK_FORCE_INLINE void unpack(hkMxVector<M>& v0, hkMxVector<M>& v1) const;
	/// Unpack only first interleaved vector and store back into \a v.
	HK_FORCE_INLINE void unpackFirst(hkMxVector<M>& v) const;
	/// Unpack only second interleaved vector and store back into \a v.
	HK_FORCE_INLINE void unpackSecond(hkMxVector<M>& v) const;
	...
	static HK_FORCE_INLINE void HK_CALL reducePrecision( hkMxVector<M>& v ); 
...
protected:
	...
	#include <Common/Base/Math/Vector/Mx/hkMxHalf8_AVX.inl>
	#else
	#include <Common/Base/Math/Vector/Mx/hkMxHalf8.inl>
	#endif
#else
	#include <Common/Base/Math/Vector/Mx/hkMxHalf8.inl>
#endif
