...
class hkIntVector
{
	public:
		...
		// todo move these two to a float2int util
		HK_FORCE_INLINE static const hkIntVector HK_CALL cast(const hkVector4& x);
		HK_FORCE_INLINE static const hkVector4 HK_CALL cast(const hkIntVector& x);
		...
		//HK_FORCE_INLINE void setMinU16( hkIntVectorParameter a, hkIntVectorParameter b );
		...
		/// saturate and cast 8 signed 32bit value to 8 unsigned signed shorts.
		...
		HK_FORCE_INLINE void broadcastComponentAtVectorMax(hkVector4Parameter v);

		/// Assuming that this = (i0, i1, i2, i3) and v = (v0, v1, v2, v3), the function will return ik
		/// where k in {0,..., 3} such that vk = max{v0, v1, v2, v3}.
		/// In case of equality, returns last index
		HK_FORCE_INLINE int getComponentAtVectorMax(hkVector4Parameter v) const;
