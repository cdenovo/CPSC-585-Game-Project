...
HK_FORCE_INLINE /*static*/ const hkIntVector HK_CALL hkIntVector::cast(const hkVector4& x)
{
	...
	const __m128 qx = _mm256_cvtpd_ps(x.m_quad);
#else
	const __m128 xy = _mm_cvtpd_ps(x.m_quad.xy);
	const __m128 zw = _mm_cvtpd_ps(x.m_quad.zw);
	const __m128 qx = _mm_shuffle_ps(xy,zw,_MM_SHUFFLE(1,0,1,0));
#endif
	i.m_quad = _mm_castps_si128(qx);
	return i;
}
...
HK_FORCE_INLINE /*static*/ const hkVector4 HK_CALL hkIntVector::cast(const hkIntVector& x)
{
	...
	__m128 x0 = _mm_castsi128_ps(x.m_quad);
#if HK_SSE_VERSION >= 0x50
	v.m_quad = _mm256_cvtps_pd(x0);
#else
	v.m_quad.xy = _mm_cvtps_pd(x0);
	v.m_quad.zw = _mm_cvtps_pd(_mm_shuffle_ps(x0,x0,_MM_SHUFFLE(1,0,3,2)));
#endif
	return v;
}
#else
...
HK_FORCE_INLINE const hkVector4Comparison hkIntVector::compareLessThanS32(hkIntVectorParameter b ) const
{
	...
	cmp.m_mask.xy = _mm_castsi128_pd(_mm_cvtepi32_epi64(m));
	cmp.m_mask.zw = _mm_castsi128_pd(_mm_cvtepi32_epi64(_mm_shuffle_epi32(m,_MM_SHUFFLE(1,0,3,2))));
#else
	__m128d xy = _mm_cvtps_pd(_mm_castsi128_ps(m));
	__m128d zw = _mm_cvtps_pd(_mm_castsi128_ps(_mm_shuffle_epi32(m,_MM_SHUFFLE(1,0,3,2))));
	cmp.m_mask.xy = _mm_cmpgt_pd(xy, _mm_setzero_pd());
	cmp.m_mask.zw = _mm_cmpgt_pd(zw, _mm_setzero_pd());
#endif
	return cmp;
#else
	return hkVector4Comparison::convert(_mm_castsi128_ps(_mm_cmplt_epi32(m_quad, b.m_quad)));
#endif
}
...
HK_FORCE_INLINE const hkVector4Comparison hkIntVector::compareEqualS32( hkIntVectorParameter b ) const
{
	...
	return cmp;
#else
	return hkVector4Comparison::convert(_mm_castsi128_ps(_mm_cmpeq_epi32(m_quad, b.m_quad)));
#endif
}
...
#else
	const __m128 xy = _mm_cvtpd_ps(selectMask.m_mask.xy);
	const __m128 zw = _mm_cvtpd_ps(selectMask.m_mask.zw);
	const __m128 v = _mm_shuffle_ps(xy,zw,_MM_SHUFFLE(1,0,1,0));
#endif
	hkQuadUint mask = _mm_castps_si128(v);
#else
