...
{
	hkClassMember::Type m_type;	///< The type of the member (can only be a basic type, integral, bool, half, float, double)
...
/// hkVariantDataUtil provides functionality to simplify the access memory as interpreted by 
/// type information. 
/// 
/// Havoks type system as defined in hkMemoryClass, can represent many different types. For example
...
/// provides methods so you can access any underlying integral type as and int, and internally 
...
/// styles of array function - ones which work on blocks of memory interpreted as arrays, and another which 
/// allow the interpretation of a piece of memory that holds a hkArray derived type.
///
/// There is some functionality revolving around 'finish'ing. During serialization a piece of memory may need 
/// to be turned into object of some type. The serialization can fill in data about the members, but can't for 
...
struct hkVariantDataUtil
{
	...
	static void HK_CALL setObject(const hkVariant& var, hkClassMember::Type type, void* data);
		/// Get an object (data points to the type in place. Ie if the type is pointer, data points to the pointer)
	...
	static hkVariant HK_CALL getVariantWithMostDerivedClass(const hkClass* klass, const void* object, const hkVtableClassRegistry* vtable, const hkClassNameRegistry* classReg);
		...
		/// set to true the pointer will have addReference called on it if thats appropriate.
	static void setPointer(const hkClass& cls, void* obj, void** dst, hkBool isReferenced = true);

		/// Calculate the size of a 'basic type' (ie not array/tuple etc). Returns 0 if couldn't determine.
