	...
	public:
		...
		struct DirectoryListing
		{
			...
			void addEntry(const Entry& ent) 
			{
				...
				e.flags = ent.flags;
				e.timeModified = ent.timeModified;
				e.size = ent.size;
			}
			...
			void addFile(const char* name, unsigned timeModified = 0, hkInt64 size = -1) 
			{
				Entry e;
				e.name = name;
				e.flags = F_ISFILE;
