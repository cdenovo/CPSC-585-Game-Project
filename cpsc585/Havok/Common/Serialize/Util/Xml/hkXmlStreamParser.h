...
class hkXmlStreamParser : public hkReferencedObject
{
	public:
		...
		static hkResult HK_CALL parseInt(const hkSubString& subString, int& valueOut);
			/// Parse an real
		static hkResult HK_CALL parseReal(const hkSubString& subString, hkFloat32& valueOut);

		static hkResult HK_CALL parseReal(const hkSubString& subString, hkDouble64& valueOut)
		{
			hkFloat32 val; 
			hkResult res = parseReal(subString, val);
			valueOut = val;
			return res;
		}
