...
template <typename T>
inline void	hkStructuredBinaryStreamWriter<SERIALIZER>::writeValue(const T& value, hkUint32 valueId)
{
...
template <typename SERIALIZER>
template <typename T>
inline void	hkStructuredBinaryStreamWriter<SERIALIZER>::writeValues(const T* values, int count, hkUint32 valueId)
{
	openScope(VALUE, valueId);
...
template <typename SERIALIZER>
template <typename T>
inline void	hkStructuredBinaryStreamWriter<SERIALIZER>::writeArray(const hkArrayBase<T>& values, hkUint32 arrayId)
{
...
//
template <typename SERIALIZER>
inline void	hkStructuredBinaryStreamWriter<SERIALIZER>::openScope(HeadType type, hkUint32 id)
{
	m_scopes.pushBack(m_stream.m_buffer.getSize());
	*((hkUint32*)m_stream.m_buffer.expandBy(4)) = (hkUint32)type;
	writeData(id);
}

//
template <typename SERIALIZER>
inline void	hkStructuredBinaryStreamWriter<SERIALIZER>::closeScope()
{
	const hkUint32 head = (hkUint32) m_scopes.back(); m_scopes.popBack();
	const HeadType type = (HeadType) (*(const hkUint32*)&m_stream.m_buffer[head]);
	write(getHead(type, m_stream.m_buffer.getSize() - head), &m_stream.m_buffer[head]);
}

//
template <typename SERIALIZER>
template <typename T>
inline void hkStructuredBinaryStreamWriter<SERIALIZER>::write(const T& src, hkUint8* dst)
{
#if HK_ENDIAN_BIG
	for(int i=0; i<(int)sizeof(T); ++i) dst[i] = ((const hkUint8*)&src)[i];
#else
	for(int i=0; i<(int)sizeof(T); ++i) dst[i] = ((const hkUint8*)&src)[sizeof(T) - i - 1];
#endif
}
