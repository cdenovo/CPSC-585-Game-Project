...
{
	/// Construtor.
	hkStructuredBinaryStreamWriter(WriterStream& stream) : hkStructuredBinaryStream(), m_stream(stream) {}
	...
	WriterStream&		getStream() { return m_stream; }

	/// Get a read-only reference to the underlying stream.
	const WriterStream&	getStream() const { return m_stream; }
	...
	inline void	writeData(hkUint8 value) { write(value, m_stream.m_buffer.expandBy(1)); }

	/// Write an hkInt8 data type.
	inline void	writeData(hkInt8 value) { write(value, m_stream.m_buffer.expandBy(1)); }

	/// Write an hkUint16 data type.
	inline void	writeData(hkUint16 value) { write(value, m_stream.m_buffer.expandBy(2)); }

	/// Write an hkInt16 data type.
	inline void	writeData(hkInt16 value) { write(value, m_stream.m_buffer.expandBy(2)); }

	/// Write an hkUint32 data type.
	inline void	writeData(hkUint32 value) { write(value, m_stream.m_buffer.expandBy(4)); }

	/// Write an hkInt32 data type.
	inline void	writeData(hkInt32 value) { write(value, m_stream.m_buffer.expandBy(4)); }

	/// Write an hkUint64 data type.
	inline void	writeData(hkUint64 value) { write(value, m_stream.m_buffer.expandBy(8)); }

	/// Write an hkInt64 data type.
	inline void	writeData(hkInt64 value) { write(value, m_stream.m_buffer.expandBy(8)); }

	/// Write an hkReal data type.
	inline void	writeData(hkReal value) { write(value, m_stream.m_buffer.expandBy(sizeof(hkReal))); }
...
private:
	...
	/// Write
	template <typename T>
	inline void write(const T& src, hkUint8* dst);
...
struct hkStructuredBinaryStreamReader : public hkStructuredBinaryStream
{
	...
	/// Return true is the end of the stream has been reached.
