															...
															hkVector4* HK_RESTRICT normalOut, hkVector4* HK_RESTRICT baryOut)
{
	...
	// Closest point is on one of the edges
	{
		...
		hkVector4 vTois;
		{
			...
			vTois.setMax(vTois, zero);
			vTois.setMin(vTois, one);
		}

		// Compute normals & distances
		hkVector4 vDistances;
		HK_ALIGN16(hkVector4) normals[3];
		{
		...
		if ( baryOut )
		{
			...
			HK_ALIGN16(hkVector4) bary[3] = { b0, b1, b2 };
...
{
	hkVector4	loBounds; loBounds.setXYZ_W(hkVector4::getConstant<HK_QUADREAL_0>(), -hkSimdReal::getConstant<HK_QUADREAL_MAX>());
	...
	hkBool32 isInside;
	if(signs.greater(loBounds).allAreSet(hkVector4Comparison::MASK_XYZ))
	{
	...
	else
	{
		...
		sols.setMax(sols, loBounds);
		sols.setMin(sols, hiBounds);
																			 ...
																			 hkVector4* HK_RESTRICT projectionOut, hkVector4* HK_RESTRICT normalOut)
{
	...
	else
	{
		...
		int					solRegion = 1;
		if(sol.isEqual(hkSimdReal::getConstant<HK_QUADREAL_0>()))		--solRegion;
		else if(sol.isEqual(hkSimdReal::getConstant<HK_QUADREAL_1>()))	++solRegion;
