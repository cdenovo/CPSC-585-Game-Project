...
namespace hkcdGsk
{
	...
	struct Cache
	{
		...
		HK_FORCE_INLINE void pack(int dimA, int dimB, int status, const hkcdVertex* HK_RESTRICT verticesOfAinA, const hkcdVertex* HK_RESTRICT verticesOfBinA);
			...
			// Unpack the cache to GSK. if used .inl file must be included explicitly.
	...
	enum GetClosestPointStatus
	{
		...
		STATUS_PD_HEURISTIC_SAMPLING,	///< The output is an approximation at best
		STATUS_PD_OUT_OF_MEMORY,		///< The output is an approximation at best
