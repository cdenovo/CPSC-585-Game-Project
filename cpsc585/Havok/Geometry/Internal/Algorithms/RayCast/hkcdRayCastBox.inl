	...
	hkVector4* HK_RESTRICT normalOut)
{
	...
	hkVector4 vB;	ray.getEndPoint(vB);
	{
		...
		if ( cmpRayOut.anyIsSet(hkVector4Comparison::MASK_XYZ) ||		// Ray is outside, no intersection!
			 !cmpRayStartOut.anyIsSet(hkVector4Comparison::MASK_XYZ)	// Ray start is completely inside, no intersection!
			)
		{
			return false;
		}
	...
	hkSimdReal fEntry, fExit;
	{
		...
		hkVector4Comparison cmp0 = vAB.notEqual(hkVector4::getConstant<HK_QUADREAL_0>());
