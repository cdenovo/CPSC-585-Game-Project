...
#include <Geometry/Internal/Types/hkcdRay.h>
	...
	/// \note The planeEquations array is read in batches of 4 vectors. Thus if numPlaneEquations is not a multiple of 4,
	/// the caller must ensure that there is sufficient padding at the end so that unmapped memory is not read.
HK_FORCE_INLINE hkBool32 HK_CALL hkcdRayCastConvex(
	const hkcdRay& ray,
