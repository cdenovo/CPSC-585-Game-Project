																		...
																		hkSimdRealParameter tolerance, hkFourTransposedPoints& normalsOut, hkVector4& fractionsInOut)
{
	...
	// Decide whether we have a valid intersection
	{
		...
		vI.setAddMul(vStart, vDir, distance);
...
{
	{
		...
		hkVector4 vAP;	vAP.setSub(ray.m_origin, vA);
		hkVector4 end0;  end0.setAddMul(vAP, ray.m_direction, fractionInOut);
		...
		hkVector4 c0;	c0.setCross(vAP, ray.m_direction);
		hkVector4 c1;	c1.setCross(vBP, ray.m_direction);
		hkVector4 c2;	c2.setCross(vCP, ray.m_direction);
...
//
HK_FORCE_INLINE hkBool32 hkcdRayTriangleIntersect(const hkcdRay& ray, const hkVector4& a, const hkVector4& b, const hkVector4& c, hkSimdReal* HK_RESTRICT fractionOut, hkVector4* HK_RESTRICT normalOut, hkBool twoSided)
{
	hkVector4	org; org.setSub(ray.m_origin, a);
	hkVector4	ab; ab.setSub(b,a);
	hkVector4	ac; ac.setSub(c,a);
	hkVector4	n; n.setCross(ab,ac);
	hkSimdReal	den = n.dot<3>(ray.m_direction);
	...
	{
		hkVector4	impact; impact.setAddMul(org, ray.m_direction, fraction);
		hkVector4	epsilon; epsilon.setAll(-1.e-8f); epsilon.mul(n.lengthSquared<3>()); // use faster length
