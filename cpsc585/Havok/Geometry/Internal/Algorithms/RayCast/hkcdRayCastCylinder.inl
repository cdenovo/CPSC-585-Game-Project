...
inline hkBool32 HK_CALL hkcdRayCastCylinder(
	const hkcdRay& ray,
	...
	hkVector4* HK_RESTRICT normalOut)
{
	...
	hkVector4Comparison capPHitFirst;
	{
		...
		const hkVector4 vAB = ray.m_direction;	// AB = B - A, B = end of the ray
		...
		dotABu.setSelect(cmpPerpendicular, hkSimdReal::getConstant<HK_QUADREAL_EPS>(), dotABu);	// Prevent division by zero
		hkSimdReal invDotABu;	invDotABu.setReciprocal(dotABu);
		...
		hkSimdReal tmp0 = hkSimdReal::getConstant<HK_QUADREAL_MAX>();
	...
	hkSimdReal t2, t3;
	{
		...
		const hkVector4 vAB = ray.m_direction;
		...
		// If (a == 0), the cylinder is parallel to the ray
