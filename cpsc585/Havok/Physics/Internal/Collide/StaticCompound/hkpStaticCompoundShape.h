...
#include <Physics/Internal/Collide/StaticCompound/hkpShapeKeyTable.h>
...
class hkpStaticCompoundShape : public hkpBvTreeShape, public hkpShapeContainer
{
	public:
		...
		struct Instance
		{
			...
			protected:
				...
				HK_FORCE_INLINE void setShapeSize( hkUint32 size );
			...
			public:
				...
				HK_FORCE_INLINE hkUint32 getShapeSize() const;
	...
	public:
		...
		HK_FORCE_INLINE const hkArray<Instance>& getInstances() const;
			...
			/// Per-instance filter info access. See set/getFilterInfoMask() for how this filter info is interpreted.
		...
		HK_FORCE_INLINE hkUint32 getInstanceFilterInfo( int instanceId ) const;
			...
			/// Per-instance filter info mask access. The actual filter info used by the engine is:
	...
	protected:
		...
		// Tree data
		HK_ALIGN16( hkUint8 m_tree[NUM_BYTES_FOR_TREE] );	//+overridetype(class hkcdStaticTree::DefaultTreeStorage6)

		friend class hkpStaticCompoundShape_Internals;
};

#include <Physics/Internal/Collide/StaticCompound/hkpStaticCompoundShape.inl>
