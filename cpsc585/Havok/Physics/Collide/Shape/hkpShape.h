...
class hkpShape : public hkReferencedObject
{
	...
	protected:	
		...
		HKP_SHAPE_VIRTUAL void castRayWithCollectorImpl( HKP_SHAPE_VIRTUAL_THIS const hkpShapeRayCastInput& input, const hkpCdBody& cdBody, hkpRayHitCollector& collector ) HKP_SHAPE_VIRTUAL_CONST = 0;
			...
			/// hkpShape::castRayBundle() interface implementation.
	...
	public:
			...
			typedef void (HK_CALL *GetSupportingVertexFunc)        (const void* thisObj, hkVector4Parameter direction, hkcdVertex &supportVertex );
			typedef void (HK_CALL *ConvertVertexIdsToVerticesFunc)	(const void* thisObj, const hkpVertexId* ids, int numIds, class hkcdVertex* verticesOut );
