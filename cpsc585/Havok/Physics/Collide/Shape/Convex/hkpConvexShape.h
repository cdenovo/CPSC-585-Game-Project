...
class hkpConvexShape : public hkpSphereRepShape
{
	public:
		...
		HK_FORCE_INLINE void getSupportingVertex( hkVector4Parameter direction, hkcdVertex& supportingVertexOut ) const;
		...
		HK_FORCE_INLINE void convertVertexIdsToVertices( const hkpVertexId* ids, int numIds, hkcdVertex* vertixArrayOut) const ;
	...
	protected:
			...
			/// Returns the vertex furthest along a given input direction.
			/// Each vertex must be identifiable by a 16bit vertex id, which must be stored in the result using hkVector4::setInt24W().
		HKP_SHAPE_VIRTUAL void getSupportingVertexImpl( HKP_SHAPE_VIRTUAL_THIS hkVector4Parameter direction, hkcdVertex& supportingVertexOut ) HKP_SHAPE_VIRTUAL_CONST = 0;

			/// Convert an array of input vertex id as returned by getSupportingVertex into an array of vertices.
			/// The .w component of the vertices out has to be set to the vertex id using hkVector4::setInt24W
		HKP_SHAPE_VIRTUAL void convertVertexIdsToVerticesImpl( HKP_SHAPE_VIRTUAL_THIS const hkpVertexId* ids, int numIds, hkcdVertex* vertixArrayOut) HKP_SHAPE_VIRTUAL_CONST = 0;
