...
//
//	Compound - AABB command

struct hkpCompoundAabbCommand
{
	public:

		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_COLLIDE, hkpCompoundAabbCommand );

	public:

		hkAabb	m_aabb;

			/// Maximum capacity of the results buffer
		int m_resultsCapacity;

			/// Pointer to the returned shape keys buffer on PPU
		hkUint32* m_results;

			/// Number of valid shape keys in the output buffer
		int m_numResultsOut;
};
