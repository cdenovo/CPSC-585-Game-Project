...
{
	m_motionState.m_linearDamping = hkFloat32(d);
}


void hkpMotion::setAngularDamping( hkReal d )
{
	m_motionState.m_angularDamping = hkFloat32(d);
}

void hkpMotion::setTimeFactor( hkReal f )
{
	m_motionState.m_timeFactor = hkFloat32(f);
}
...
{
	m_gravityFactor = hkFloat32(gravityFactor);
}
...
void hkpMotion::requestDeactivation()
{
	...
	m_deactivationRefPosition[0](3) = HK_REAL_MAX;	// speed
