...
class hkpBreakableMaterial : public hkReferencedObject
{
	...
	public:
		...
		class ShapeKeyCollector
		{
			public:
				...
				HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_DESTRUCTION, hkpBreakableMaterial::ShapeKeyCollector);
	...
	public:
		...
		virtual void createInverseMapping(const hkpShape* shape) {}
		...
		virtual MaterialId getSubShapeMaterialIndex(const hkpShape* rootShape, hkUint32 subShapeId) const { return INVALID_MATERIAL_ID; }

			/// Returns the material set on the given shape key
		virtual hkpBreakableMaterial* getShapeKeyMaterial(const hkpShape* shapePpu, hkpShapeKey subShapeKey) const { return HK_NULL; }
		...
		virtual void disableSubShapes(hkpShape* rootShape, const MaterialId* subMaterialIndices, int numSubMaterialIndices) {}
		...
		virtual void getShapeKeysForSubShapes(const hkpShape* rootShape, const hkUint32* subShapeIdPtr, int numSubShapeIds, ShapeKeyCollector* collector) const {}

			/// Appends the sub-material indices set on the given sub-shapes to the given array
		virtual void getSubShapeMaterialIndices(const hkpShape* rootShape, const hkArray<hkUint32>& subShapeIdsIn, hkArray<MaterialId>& subMaterialsOut) const {}
		...
		HK_FORCE_INLINE const Properties* getProperties() const;
			...
			/// Sets the properties
		HK_FORCE_INLINE void setProperties(Properties* newProperties);
	...
	protected:
			...
			/// Material properties
		hkRefPtr<Properties> m_properties;
};

#include <Physics/Dynamics/Destruction/BreakableMaterial/hkpBreakableMaterial.inl>
