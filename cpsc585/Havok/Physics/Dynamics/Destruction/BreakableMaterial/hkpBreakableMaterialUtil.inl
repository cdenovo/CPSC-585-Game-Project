...
inline void HK_CALL hkpBreakableMaterialUtil::downloadShapeOnSpu(const hkpShape* shapePpu, hkpShape*& shapeSpu)
{
	hkpShape* rootShapePpu = const_cast<hkpShape*>(shapePpu);
	bool shapeIsMopp = true;
	do
	{
		HK_DOWNLOAD_DESTRUCTION_PTR_TO_SPU(shapeSpu, rootShapePpu, MAX_SHAPE_SIZE);

		shapeIsMopp = ( shapeSpu->getType() == HK_SHAPE_MOPP );
		if ( shapeIsMopp )
		{
			...
			rootShapePpu = const_cast<hkpShape*>(moppSpu->getChild());
		}
	} while ( shapeIsMopp );
}
...
#define OFFSET_OF_EMS_SHAPE_INFO	2
HK_COMPILE_TIME_ASSERT(HK_OFFSET_EQUALS(hkpExtendedMeshShape::Subpart, m_shapeInfo, OFFSET_OF_EMS_SHAPE_INFO));

//
//	Returns the material strength set on the given sub-shape. Returns true if the material is breakable, false otherwise.

inline hkBool HK_CALL hkpBreakableMaterialUtil::getSubShapeMaterialStrength(const hkpBreakableMaterial* materialSpu, const hkpShape* shapePpu, hkpShapeKey shapeKey, hkUFloat8& materialStrengthOut)
{
	...
	if ( mtlMapping != hkpBreakableMaterial::MATERIAL_MAPPING_NONE )
	{
		...
		HK_DECLARE_SPU_LOCAL_DESTRUCTION_PTR(hkpShape, shapeSpu, MAX_SHAPE_SIZE);
		...
		MaterialId subMtlId;
		{
			...
			case hkpBreakableMaterial::MATERIAL_MAPPING_LIST_SHAPE:	// This is a hkpListShapeBreakableMaterial
				{
					HK_ASSERT2(0x623113be, shapeSpu->getType() == HK_SHAPE_LIST, "Failed to access the hkpListShape");
			...
			case hkpBreakableMaterial::MATERIAL_MAPPING_EMS:		// This is a hkpExtendedMeshShapeBreakableMaterial
				{		
					HK_ASSERT2(0x623113be, shapeSpu->getType() == HK_SHAPE_EXTENDED_MESH, "Failed to access the hkpExtendedMeshShape");
			...
			case hkpBreakableMaterial::MATERIAL_MAPPING_STATIC_COMPOUND:	// This is a hkpStaticCompoundShapeBreakableMaterial
				{
					HK_ASSERT2(0x623113be, shapeSpu->getType() == HK_SHAPE_STATIC_COMPOUND, "Failed to access the static compound shape");
...
#undef OFFSET_OF_EMS_SHAPE_INFO


//
//	Returns the index of the sub-material belonging to the given sub-shape

inline hkpBreakableMaterialUtil::MaterialId HK_CALL hkpBreakableMaterialUtil::getSubShapeMaterialIndex(const hkpBreakableMaterial* rootMtl, const hkpShape* rootShape, hkUint32 subShapeId)
{
...
inline hkpBreakableMaterial* HK_CALL hkpBreakableMaterialUtil::getMaterialAtShapeKey(const hkpBreakableMaterial* materialSpu, const hkpShape* shapePpu, hkpShapeKey shapeKey)
{
...
inline void HK_CALL hkpBreakableMaterialUtil::getSubShapeMaterialIndices(const hkpBreakableMaterial* rootMtl, const hkpShape* rootShape, const hkArray<hkUint32>& subShapeIdsIn, hkArray<MaterialId>& subMaterialsOut)
{
