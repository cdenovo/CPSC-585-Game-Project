...
class hkpListShapeBreakableMaterial : public hkpBreakableMultiMaterial
{
	...
	public:			
		...
		virtual MaterialId getSubShapeMaterialIndex(const hkpShape* rootShape, hkUint32 subShapeId) const;

			/// Returns the material set on the given shape key
		virtual hkpBreakableMaterial* getShapeKeyMaterial(const hkpShape* shapePpu, hkpShapeKey subShapeKey) const;
	...
	protected:
		...
		static HK_FORCE_INLINE const hkpListShape* HK_CALL getListShape(const hkpShape* shape);

};

#	include <Physics/Dynamics/Destruction/BreakableMaterial/MultiMaterial/ListShape/hkpListShapeBreakableMaterial.inl>
